...
def preprocess():
    ...
    print("feature reduction : ", feature_reduction)

    global selected_features
    selected_features = []

    for fr in range(0,feature_reduction.size):
        if feature_reduction[fr] == False:
            selected_features.append(fr)

    print("select feature indices : ", selected_features)
    print("selected feature size : ", np.size(selected_features))
...
def nnPredict(w1, w2, data):
    ...
    # Your code here
    # Input layer
    input_bias = np.ones(len(data))
    input_data = np.column_stack([data, input_bias])

    # Hidden Layer
    netj = np.dot(input_data, np.transpose(w1))
    zj = sigmoid(netj)

    zj_bias = np.ones(len(input_data))
    zj = np.column_stack([zj, zj_bias])
    ...
    for row in range(o.shape[0]):
        labels[row] = int(math.floor(np.argmax(o[row])))
...
new_args = (n_input, n_hidden, n_class, train_data, train_label, lambdaval)
